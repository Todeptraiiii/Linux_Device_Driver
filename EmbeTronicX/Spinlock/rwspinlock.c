...
int thread_function2(void *pv) // dong thoi
{
    while(!kthread_should_stop())
    {
        ...
        pr_info("Dang trong Thread Function 2: Read value %lu \n", etx_global_variable);
...
/* Thread function 3*/
int thread_function3(void *pv) // dong thoi
{
    while(!kthread_should_stop())
    {
        read_lock(&etx_rwlock);
        pr_info("Dang trong Thread Function 3: Read value %lu \n", etx_global_variable);
        read_unlock(&etx_rwlock);
        msleep(1000);
    }
    return 0;
...
static int __init etx_driver_init(void)
{
        ...
        if(etx_thread3)
        {
            pr_err("Kthread3 duoc tao thanh cong ... \n");
        } 
        else
        {
            pr_err("Khong the tao kthread3 \n");
            goto r_device;
        }
        return 0;
