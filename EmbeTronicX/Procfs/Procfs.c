...
static struct proc_dir_entry *parent; // Bien struct tao duong dan
...
/*Nhung function nay se duoc goi khi chung ta mo procfs file*/
static int open_proc(struct inode *inode, struct file *file)
{
...
/* Ham nay se duoc goi khi chung ta ghi vao procfs file*/
static ssize_t write_proc(struct file *filp, const char *buff, size_t len, loff_t *off)
{
    pr_info("Proc file da duoc ghi...\n");
...
static int __init etx_driver_init(void)
{
    ...
    if(parent == NULL)
    {
        pr_info("Error tao proc entry");
    }
    /* Tao file proc hay proc entry /proc/etx/*/
